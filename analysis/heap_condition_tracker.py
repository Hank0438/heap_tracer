...
class MallocInspect(SimProcedure):
    ...
    def check_malloc(self, size, malloc_addr, vulns=None, ctrl_data=None): #pylint:disable=unused-argument
        # Clear breakpoints
        print("chunk addr: ", self.state.regs.rax)
        ...
        # Don't track heap_init malloc (0x200)
        ...
        print("ctrl_data_idx: ", self.state.heaphopper.ctrl_data_idx)
            ...
            if not found:
                for key in list(self.state.heaphopper.free_dict.keys()):
                    ...
                    if val == sol:
                        ...
                        print("double free:", self.state.heaphopper.double_free)
                        # input("@")
